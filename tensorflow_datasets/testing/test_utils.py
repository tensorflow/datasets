...
@contextlib.contextmanager
def mock_kaggle_api(competition=None, err_msg=None):
  """Mock out the kaggle CLI.

  Args:
    competition: `<str>`, Name of the competition/dataset.
  ...
  def make_mock_download_call(competition, err_msg):
    ...
    def check_output(command_args):
      ...
      out_dir = command_args[command_args.index('-p') + 1]
      fpath = os.path.join(out_dir, competition)
      with tf.io.gfile.GFile(fpath, 'w') as f:
        f.write(competition)
      return tf.compat.as_bytes('Downloading %s to %s' % (competition, fpath))

    return check_output

  with absltest.mock.patch('subprocess.check_output',
                           make_mock_download_call(competition, err_msg)):
    yield
