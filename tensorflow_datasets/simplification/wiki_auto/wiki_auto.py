...
_URLs = {
  ...
  'auto': {
    'part_1':
      'https://github.com/chaojiang06/wiki-auto/raw/master/wiki-auto/all_data/wiki-auto-part-1-data.json',
    'part_2':
      'https://github.com/chaojiang06/wiki-auto/raw/master/wiki-auto/all_data/wiki-auto-part-2-data.json',
  },
}
...
class WikiAuto(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      tfds.core.BuilderConfig(name='manual',
        description='A set of 10K Wikipedia sentence pairs aligned \
by crowd workers.'),
    ...
    if self.builder_config.name in ['manual', 'auto']:
      return [
        tfds.core.SplitGenerator(
          name=spl,
          gen_kwargs={'filepaths': data_dir, 'split': spl}
        )
        for spl in data_dir
      ]
    else:
        ...
        res = {
          ...
          'normal': {
            ...
            'normal_article_content': {
              'normal_sentence_id': [
                sen_id for sen_id, sen_txt in
                  example_dict['normal']['content'].items()
              ],
              'normal_sentence': [
                sen_txt for sen_id, sen_txt in
                  example_dict['normal']['content'].items()
              ],
            },
          },
          'simple': {
            ...
            'simple_article_content': {
              'simple_sentence_id': [
                sen_id for sen_id, sen_txt in
                  example_dict['simple']['content'].items()
              ],
              'simple_sentence': [
                sen_txt for sen_id, sen_txt in
                  example_dict['simple']['content'].items()
          ...
          'paragraph_alignment': {
            'normal_paragraph_id': [
              norm_id for simp_id, norm_id in
                example_dict.get('paragraph_alignment', [])
            ],
            'simple_paragraph_id': [
              simp_id for simp_id, norm_id in
                example_dict.get('paragraph_alignment', [])
            ],
          },
          'sentence_alignment': {
            'normal_sentence_id': [
              norm_id for simp_id, norm_id in
                example_dict.get('sentence_alignment', [])
            ],
            'simple_sentence_id': [
              simp_id for simp_id, norm_id in
                example_dict.get('sentence_alignment', [])
