...
@inproceedings{alva-manchego-etal-2020-asset,
    title = "{ASSET}: {A} Dataset for Tuning and Evaluation of Sentence \
Simplification Models with Multiple Rewriting Transformations",
...
_DESCRIPTION = """\
ASSET is a dataset for evaluating Sentence Simplification systems with \
...
_URL_LIST = [
  ('human_ratings.csv', 'https://github.com/facebookresearch/asset/raw\
/master/human_ratings/human_ratings.csv'),
  ('asset.valid.orig', 'https://github.com/facebookresearch/asset/raw\
/master/dataset/asset.valid.orig'),
  ('asset.test.orig', 'https://github.com/facebookresearch/asset/raw\
/master/dataset/asset.test.orig'),
]
_URL_LIST += [
  (
    f'asset.{spl}.simp.{i}',
    f'https://github.com/facebookresearch/asset/raw/master\
/dataset/asset.{spl}.simp.{i}',
  )
...
class Asset(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
    tfds.core.BuilderConfig(name='simplification',
      version=VERSION,
      description='A set of original sentences aligned with 10 possible \
simplifications for each.'
    ),
    tfds.core.BuilderConfig(name='ratings',
      version=VERSION,
  ...
  def _generate_examples(self, filepaths, split):
    ...
    if self.builder_config.name == 'simplification':
      ...
      for id_, lines in enumerate(zip(*files)):
        yield id_, {'original': lines[0].strip(),
