...
class WmtT2tTranslate(wmt.WmtTranslate):
  ...
  def _subsets(self):
    return {
        tfds.Split.TRAIN: ["europarl_v7", "commoncrawl", "newscommentary_v13"],  # pyrefly: ignore[missing-attribute]
        tfds.Split.VALIDATION: ["newstest2013"],  # pyrefly: ignore[missing-attribute]
        tfds.Split.TEST: ["newstest2014"],  # pyrefly: ignore[missing-attribute]
    }
