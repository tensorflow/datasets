...
class DatasetBuilder(object):
  ...
  **Configuration**: Some `DatasetBuilder`s expose multiple variants of the
  ...
  def _build_single_dataset(self, split, shuffle_files, batch_size, decoders,
    ...
    # Apply mapping to the decoded data
    if map_fns is not None:
      map_fns = functools.partial(transform_lib.partial_map_nested, map_fns)
      ds = ds.map(map_fns, num_parallel_calls=tf.data.experimental.AUTOTUNE)
  ...
  @abc.abstractmethod
  def _as_dataset(self, split, decoders=None, map_fns=None, shuffle_files=None):
    """Constructs a `tf.data.Dataset`.
    ...
    Args:
      ...
      decoders: Nested structure of `Decoder` object to customize the dataset
        decoding.
      map_fns: Nested structure of transformation function to apply after
        decoding.
