...
class DatasetBuilder(metaclass=abc.ABCMeta):
  ...
  **Configuration**: Some `DatasetBuilder`s expose multiple variants of the
  ...
  # Named configurations that modify the data generated by
  ...
  def __init__(self,
    ...
    Args:
      ...
      version: `str`. Optional version at which to load the dataset. An error 
        is raised if specified version cannot be satisfied. Eg: '1.2.3',
        '1.2.*'. The special value "experimental_latest" will use the highest
        version, even if not default. This is not recommended unless you know 
        what you are doing, as the version could be broken.
    """
    # For pickling:
  ...
  def download_and_prepare(
    ...
    Args:
      ...
      download_config: `tfds.download.DownloadConfig`, further configuration
        for downloading and preparing dataset.
    ...
    if self.version.tfds_version_to_prepare:
      ...
      raise AssertionError(
          ...
          "be generated using TFDS code synced @ {} or earlier. Either sync "
          "to that version of TFDS to first prepare the data or use another "
    ...
    with file_format_adapter.incomplete_dir(self._data_dir) as tmp_data_dir:
      ...
      with utils.temporary_assignment(self, "_data_dir", tmp_data_dir):
        if(download_config.try_download_gcs and
          ...
          # DatasetInfo.read_from_directory to possibly restore these
          # attributes when reading from package data.
  ...
  def as_dataset(self,
    ...
    Args:
      split: `tfds.core.SplitBase`, which subset(s) of the data to read. If
        None (default), returns all splits in a dict
        `<key: tfds.Split, value: tf.data.Dataset>`.
      batch_size: `int`, batch size. Note that variable-length features will
        be 0-padded if `batch_size` is set. Users that want more custom
        behavior should use `batch_size=None` and use the `tf.data` API to
        construct a custom pipeline. If `batch_size == -1`, will return
        feature dictionaries of the whole dataset with `tf.Tensor`s instead
      ...
      decoders: Nested dict of `Decoder` objects which allow to customize the
        ...
        [the guide]
        (https://github.com/tensorflow/datasets/tree/master/docs/decode.md)
        for more info.
  ...
  def _download_and_prepare(
    ...
    Args:
      dl_manager: (DownloadManager) `DownloadManager` used to download and
        cache data.
  ...
  def _create_builder_config(self,
    ...
    if builder_config is None and self.BUILDER_CONFIGS:
      ...
      logging.info("No config specified, defaulting to first: %s/%s",
                   self.name,
                   builder_config.name)
...
class FileAdapterBuilder(DatasetBuilder):
  ...
  def _split_generators(
    ...
    Note that the `DownloadManager` caches downloads, so it is fine to have
    each generator attempt to download the source data.
  ...
  def _make_split_generators_kwargs(
    ...
    """
    Get kwargs for `self._split_generators()` from
...
class GeneratorBasedBuilder(FileAdapterBuilder):
    ...
    Yields:
      key: `str` or `int`, a unique deterministic example identification key.
        ...
        * Deterministic: When generating the dataset twice, the same example
        ...
        from a text file.
        The key will be hashed and sorted to shuffle examples
        deterministically, such as generating the dataset multiple times keep
  ...
  def _prepare_split_legacy(self,
                            generator: splits_lib.SplitGenerator,
