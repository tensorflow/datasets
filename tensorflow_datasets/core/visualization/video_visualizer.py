...
def display_gif_grid(width: int, height: int, rows: int, cols: int) -> None:
...
def _display_gif(gif_fn, ds: tf.data.Dataset, # pylint: disable=too-many-arguments
...
def _write_video_gif(video: np.ndarray, fps: int, index: int) -> None:
...
class VideoGridVisualizer(visualizer.Visualizer):
    ...
    def show( # pylint: disable=too-many-arguments
        ...
        def make_cell_fn(ex: np.ndarray, index: int) -> None:
            ...
            _write_video_gif(ex[video_key], fps, index)
        # Display GIF
