...
def mock_gfile_pathlike():
  ...
  Before TF 2.3, GFile API is not PathLike compatible.
  After TF2.3, this function is a no-op.
  ...
  print("******", tf_version, min_tf_version)
  if tf_version >= min_tf_version:
    print("SKIP **************")
    ...
    with contextlib.ExitStack() as stack:
      ...
      import pathlib
      assert not tf.io.gfile.exists(pathlib.Path("/tmp/osadasdasda/tfds"))
      yield
