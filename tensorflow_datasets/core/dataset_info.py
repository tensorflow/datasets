...
def _nest_to_proto(nest: Nest):
  """Creates a `struct_pb2.Value` from a limited `tf.nest` style structure.
  ...
  if nest_type is tuple:
    proto = struct_pb2.ListValue()
    for item in nest:
      proto.append(_nest_to_proto(item))
  elif nest_type is dict:
    proto = struct_pb2.Struct()
    proto.update({key: _nest_to_proto(value) for key, value in nest.items()})
...
def _supervised_keys_to_proto(
    keys: Union[Tuple[Nest, Nest], Tuple[Nest, Nest, Nest]]
  ...
  proto.tuple.extend(_nest_to_proto(key) for key in keys)
  return proto


def _nest_from_proto(proto) -> Nest:
  ...
  if isinstance(proto, str):
    return proto
  elif isinstance(proto, struct_pb2.ListValue):
    return tuple(_nest_from_proto(item) for item in proto)
  elif isinstance(proto, struct_pb2.Struct):
    return {key: _nest_from_proto(value) for key, value in proto.items()}
  else:
...
def _supervised_keys_from_proto(
    proto: dataset_info_pb2.SupervisedKeys
) -> Union[Tuple[Nest, Nest], Tuple[Nest, Nest, Nest]]:
  ...
  if proto.input and proto.output:
    ...
    return tuple(_nest_from_proto(item) for item in proto.tuple)
  else:
