    ...
    def __init__(
        ...
        Args:
            ...
            download_mode: `tfds.GenerateMode`, how to deal with downloads or data that
                            already exists. Defaults to `REUSE_DATASET_IF_EXISTS`, which
                            will reuse both downloads and data if it already exists.
...
class DownloadManager:
    ...
    def _handle_download_result(self, resource: resource_lib.Resource, tmp_dir_path: str,
        ...
        * Eventually rename `url_path` -> `file_path` when `record_checksums=True`
        ...
        resource_lib.write_info_file(resource=resource, path=url_path,
        ...
        # Even if `_handle_download_result` is executed asynchronously, Python
    ...
    def _save_url_info_and_rename(self, url: str, url_path: str, url_info: checksums.UrlInfo
        ...
        # Rename (after checksum got saved successfully)
    ...
    def download_checksums(self, checksums_url):
        ...
        with tf.io.gfile.GFile(checksums_path) as file:
            self._url_infos.update(checksums.parse_url_infos(file))
...
def _read_url_info(url_path: str) -> checksums.UrlInfo:
    ...
    Raises:
        ValueError: If 'url_info' is not found in the .INFO file. This likely indicates
                    the files were downloaded with a previous version of TFDS (<=3.1.0).
    """
    file_info = resource_lib.read_info_file(url_path)
    if 'url_info' not in file_info:
        raise ValueError(
            'Could not found `url_info` in {}. This likely indicates that '
            'the files were downloaded with a previous version of TFDS (<=3.1.0). '
        )
...
if sys.version_info[0] > 2:
    def _wait_on_promise(prom):
        return prom.get()
else:
    def _wait_on_promise(prom):
        while True:
            result = prom.get(sys.maxint)  # pylint: disable=g-deprecated-member-used
            if prom.is_fulfilled:
                return result
# ============================================================================
