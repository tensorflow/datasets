...
class FeatureConnector(object):
  ...
  def from_json_content(
      cls: Type[T],
      value: Union[Json, feature_pb2.Feature],
  ) -> T:
    ...
    if isinstance(value, dict):
      init_kwargs = value
    elif isinstance(value, feature_pb2.JsonFeature):
      init_kwargs = json.loads(value.json)
    else:
      raise TypeError(f'Unexpected feature connector value: {value!r}')
    return cls(**init_kwargs)  # pytype: disable=not-instantiable
  ...
  def to_json_content(self) -> Union[Json, message.Message]:
    ...
    """
    return feature_pb2.JsonFeature(json='{}')
  ...
  def from_config(cls, root_dir: str) -> 'FeatureConnector':
    ...
    with tf.io.gfile.GFile(make_config_path(root_dir)) as f:
      ...
      print(f'XDEBUG: feature.py.from_config json_value={json_value}')
      feature = FeatureConnector.from_json(json_value)
      print(f'XDEBUG: feature.py.from_config feature={feature}')
...
def _name2proto_cls(cls_name: str) -> Type[message.Message]:
  ...
  oneof_descriptor = feature_pb2.Feature.DESCRIPTOR.oneofs_by_name['content']
  all_cls_descriptors = [f.message_type for f in oneof_descriptor.fields]
  name2cls = {
...
def _proto2oneof_field_name(proto):
  """Returns the field name associated with the class."""
  oneof_descriptor = feature_pb2.Feature.DESCRIPTOR.oneofs_by_name['content']
  cls2oneof_field_name = {
      f.message_type._concrete_class: f.name for f in oneof_descriptor.fields  # pylint: disable=protected-access
  }
  return cls2oneof_field_name[type(proto)]
