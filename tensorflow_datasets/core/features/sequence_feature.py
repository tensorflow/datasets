...
class Sequence(feature_lib.FeatureConnector):
  ...
  def _should_forward_transform(self, transform):
    """Checks if the transformation should be forwarded to the sequence."""
    return self._seq_feature['inner']._should_forward_transform(transform)   # pylint: disable=protected-access
