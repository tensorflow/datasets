...
class Holder(object):
  """Abstract base class for holder types."""
  ...
  def create_basedir(self):
    """Creates output path folder of fake examples."""
...
class ImageHolder(Holder):
  """Basic holder and generator of image types."""
  ...
  def image_size(self):
    """Measures the edges of the image."""
    try:
      if self.zip_file:
        infile = self.path
...
class PlainTextHolder(Holder):
  """Basic holder and generator of text types."""
...
class ZipHolder(Holder):
  """Basic holder and generator of compressed file types."""

  def __init__(self, *args, **kwargs):
    super(ZipHolder, self).__init__(*args, **kwargs)

  def create_fakes(self):
    """Creates fakes of compressed files."""
    if self.typ == 'gztar':
    ...
    for prefix in folders[:2]:  # take 2 example from 2 folders
      ex_files += list(filter(lambda x: not os.path.basename(x).startswith('.')
                              and x.startswith(prefix), f))[1:3]  # pylint: disable=cell-var-from-loop
      ex_files += list(filter(lambda x: not os.path.basename(x).startswith('.')
                              and x.startswith(prefix)  # pylint: disable=cell-var-from-loop
...
class HolderFactory(Holder):
  ...
  def generate_holder(self):
    if self.typ in ('zip', 'gztar'):  # pylint: disable=no-else-return
      ...
      return PlainTextHolder(self.zip_file, self.name, self.typ, self.path,
                             self.output_path)
    else:
      print(self.path, " is not created because it's not a supported type")
...
class Generator(object):
  ...
  def __init__(self, dataset_name, dataset_path=None, dataset_type='image'):
    ...
    self.inpath = dataset_path if dataset_path else dataset_folder_finder(dataset_name)[0]  # pylint:disable=line-too-long
    ...
    self.is_extracted = (False if dataset_path else
                         dataset_folder_finder(dataset_name)[1] == 'extracted')

  def zip_generator(self):
    """Generator of compressed files."""
    self.outpath = os.path.join(self.fake_examples_dir,
                                os.path.basename(self.inpath))

    if self.inpath.endswith('.tar.gz'):
      hold = HolderFactory(None, self.dataset_name, 'gztar', self.inpath,
                           self.outpath)
    elif self.inpath.endswith('.zip'):
      hold = HolderFactory(None, self.dataset_name, 'zip', self.inpath,
                           self.outpath)
  ...
  def generator(self):
    ...
    if not os.path.isdir(self.inpath):
      ...
      for dirpath, _, filenames in tf.io.gfile.walk(self.inpath):
        out_folder_path = os.path.join(self.outpath,
                                 os.path.relpath(dirpath, self.inpath))
        if not tf.io.gfile.isdir(out_folder_path):
          tf.io.gfile.mkdir(out_folder_path)
        else:
        ...
        while count < 2:  # take just 2 files on one folder
          ...
          file_target_path = os.path.join(out_folder_path, file)
