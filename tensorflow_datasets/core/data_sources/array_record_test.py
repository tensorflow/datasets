...
def test_data_source_calls_array_record_data_source():
  ...
  with mock.patch.object(
      ...
      return_value=['deserialized examples'],
    ...
    assert data_source[[0, 12]] == [['deserialized examples']]
    assert data_source[12] == ['deserialized examples']
    deserialize_example_mock.assert_called_with(
        ['serialized examples'], decoders=None
    )
    assert deserialize_example_mock.call_count == 2
