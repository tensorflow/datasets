...
class Cifar10H(tfds.core.GeneratorBasedBuilder):
        ...
        features=tfds.features.FeaturesDict(
            {
                "id":
                    tfds.features.Text(),
                "image":
                    tfds.features.Image(shape=_CIFAR_IMAGE_SHAPE),
                "label":
                ...
                "trial_indices":
                    tfds.features.Sequence(
                        feature=tfds.features.Scalar(dtype=tf.int32)
                    ),
                "annotator_ids":
                    tfds.features.Sequence(
                        feature=tfds.features.Scalar(dtype=tf.int32)
                ...
                "reaction_times":
                    tfds.features.Sequence(
                        feature=tfds.features.Scalar(dtype=tf.float32)
                    ),
            }
        ),
  ...
  def _cifar_info(self):
    return Cifar10HInfo(
        ...
        "https://github.com/jcpeterson/cifar-10h/raw/master/data/cifar10h-raw.zip",
        train_files=[
            "data_batch_1.bin",
            "data_batch_2.bin",
            "data_batch_3.bin",
      ...
      for labels, np_image in _load_data(path, len(label_keys)):
        ...
        if "test" in split_prefix:
          ...
          record["soft_label"] = tf.reduce_mean(
              tf.one_hot(record["human_labels"], 10, dtype=tf.float32), axis=0
          ).numpy()
          ...
          record["soft_label"] = tf.one_hot(
              record["label"], 10, dtype=tf.float32
          ).numpy()
...
def _get_field_for_example(data, example_id, field):
  ...
  return data[data["cifar10_test_test_idx"] == example_id][field].values
