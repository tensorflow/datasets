...
def convert_metadata(
  ...
  has_split_with_missing_shards = False
  for split_info in info.splits.values():
    available_shards = split_info.get_available_shards(out_path)
    if len(available_shards) < split_info.num_shards:
      has_split_with_missing_shards = True
      logging.warning(
  ...
  if in_dir == out_path:
    if has_split_with_missing_shards:
      logging.warning(
          'Skipping updating metadata because some shards are missing.'
      )
      return
