...
def _ffmpeg_decode_diskless(
  ...
  """
  with tf.io.gfile.Open(path, 'rb') as f:
    data = f.read()
  if not data:
...
class WebVid(tfds.core.GeneratorBasedBuilder):
  """The WebVid Dataset."""
  ...
  def _info(self) -> tfds.core.DatasetInfo:
    ...
    features = {
        'video': tfds.features.Video(
            video_shape, encoding_format=self._encoding_format
        ),
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    def get_captions_path(glob):
      print(os.path.join(dl_manager.manual_dir, glob))
      paths = tf.io.gfile.glob(os.path.join(dl_manager.manual_dir, glob))
      if len(paths) != 1:
    ...
    return {
        'train': self._generate_examples(
            captions_csv_path=get_captions_path(self._train_captions_glob),
            image_base_path=dl_manager.manual_dir,
        ),
        'validation': self._generate_examples(
            captions_csv_path=get_captions_path(self._val_captions_glob),
            image_base_path=dl_manager.manual_dir,
        ),
    }
  ...
  def _generate_examples(self, captions_csv_path, image_base_path):
    ...
    def _load_csv(path):
      with tf.io.gfile.GFile(path, 'r') as csv:
        # with open(path, 'r') as csv:
        # TODO(hmoraldo): replace io.StringIO(csv.read()) by csv, once
        # GFile.readable() is added.
        # Read all columns as strings, to avoid buggily converting ids with
        # leading zeros into ints.
        df = pd.read_csv(
            io.StringIO(csv.read()), sep=',', encoding='utf-8', dtype='str'
        )
        return df
    ...
    def _process_example(data_row):
      ...
      file_path = data_row[self._video_path_key]
      new_video_id = data_row[self._new_video_id_key]
      caption = data_row[self._caption_key]
      url = data_row[self._url_key]
      ...
      extra_input_args = [
          ...
          '-vf',
          f'fps={self._fps},scale={width}:{height}:force_original_aspect_ratio=increase,crop={width}:{height}',  # pylint: disable=line-too-long
          '-sws_flags',
      ...
      try:
        ...
        frames = _ffmpeg_decode_diskless(
            ...
            num_target_frames=self._max_frames,
    ...
    files = tf.io.gfile.glob(f'{image_base_path}/*_*/*.mp4')
    df_files = pd.DataFrame(list(files), columns=[self._video_path_key])
    logging.info('df_files.shape %s', df_files.shape)
    print(df_files.to_dict('records'))

    # Read info CSV.
    df_info = _load_csv(captions_csv_path)
    logging.info('df_info.shape %s', df_info.shape)
    print(df_info.to_dict('records'))

    def path_to_id(path):
      return os.path.splitext(os.path.join(*pathlib.Path(path).parts[-2:]))[0]

    # Create base names to join
    df_info[self._new_video_id_key] = (
        df_info[self._page_dir_key] + '/' + df_info[self._video_id_key]
    )
    df_files[self._new_video_id_key] = df_files[self._video_path_key].map(
        path_to_id
    )
    print(df_info.to_dict('records'))
    print(df_files.to_dict('records'))
    ...
    # Merge the two
    df = df_files.merge(df_info, on=self._new_video_id_key)
