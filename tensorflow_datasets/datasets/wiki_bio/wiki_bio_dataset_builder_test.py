...
from tensorflow_datasets.datasets.wiki_bio import wiki_bio_dataset_builder
...
class WikiBioTest(testing.DatasetBuilderTestCase):
  DATASET_CLASS = wiki_bio_dataset_builder.Builder
  SPLITS = {
      'train': 3,
      'validation': 3,
      'test': 3,
  ...
  def test_get_table(self):
    self.assertCountEqual(
        {dataset_name}_dataset_builder._get_table(_INFO_BOX_TEST),
        [
            {
                'column_header': 'name',
                'row_number': 1,
