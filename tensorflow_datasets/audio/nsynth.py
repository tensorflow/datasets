...
(Kim et al., 2018) and loudness, measured at 250 Hz.
"""
...
class NsynthConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self,
               iclr2019_gansynth_subset=False,
               estimate_f0_and_loudness=False,
               **kwargs):
    """Constructs a NsynthConfig.

    Args:
      iclr2019_gansynth_subset: bool, whether to use the subset of the dataset
        introduced in the ICLR 2019 GANSynth paper (Engel, et al. 2018). This
        subset uses acoustic-only instrument sources and limits the pitches to
        the interval [24, 84]. The train and test splits are also modified so
        that instruments (but not specific notes) overlap between them. See
    ...
    """
    name_parts = []
    if iclr2019_gansynth_subset:
      name_parts.append("iclr2019_gansynth_subset")
    else:
    ...
    super(NsynthConfig, self).__init__(
        ...
        **kwargs)
    self.iclr2019_gansynth_subset = iclr2019_gansynth_subset
...
class Nsynth(tfds.core.BeamBasedBuilder):
  ...
  def _info(self):
    features = {
        "id":
            tf.string,
        "audio":
            tfds.features.Sequence(tf.float32, _AUDIO_RATE * _NUM_SECS),
        "pitch":
    ...
    if self.builder_config.estimate_f0_and_loudness:
      features["f0"] = tfds.features.Sequence({
          "hz": tf.float32,
          "midi": tf.int32,
          "confidence": tf.float32,
      }, length=_F0_AND_LOUDNESS_RATE * _NUM_SECS + 1)
      features["loudness_db"] = tfds.features.Sequence(
          tf.float32, length=_F0_AND_LOUDNESS_RATE * _NUM_SECS + 1)
  ...
  def _split_generators(self, dl_manager):
    ...
    if self.builder_config.iclr2019_gansynth_subset:
      dl_urls["gansynth_splits"] = (
          _BASE_DOWNLOAD_PATH + "gansynth_iclr_2019_splits.csv")
  ...
  def _build_pcollection(self, pipeline, tfrecord_dirs, ids, split):
    ...
    def _estimate_f0(ex):
      ...
      _, f0_hz, f0_confidence, _ = tfds.core.lazy_imports.crepe.predict(
          ...
          verbose=0)
      f0_midi = tfds.core.lazy_imports.librosa.core.hz_to_midi(f0_hz) / 127
    ...
    def _compute_loudness(ex):
      ...
      mean_loudness_db = librosa.amplitude_to_db(
          ...
          amin=amin,
          top_db=top_db)
      ex["loudness_db"] = mean_loudness_db
      return ex

    examples = (
        [pipeline  # pylint:disable=g-complex-comprehension
         | "read_tfrecord_%d" % i >> beam.io.ReadFromTFRecord(
             os.path.join(dir_, "*tfrecord*"),
             coder=beam.coders.ProtoCoder(tf.train.Example))
         for i, dir_ in enumerate(tfrecord_dirs)]
        | beam.Flatten()
    ...
    if self.builder_config.estimate_f0_and_loudness:
      ...
      if split == tfds.Split.TRAIN:
        ...
        loudness = examples | beam.Map(lambda x: np.mean(x["loudness_db"]))
        loudness_mean = (
            loudness
