    ...
    def _info(self):
        return tfds.core.DatasetInfo(
            ...
            description=(
                ...
                "in the field and is referenced frequently to this day. "
                "(See Duda & Hart, for example.) The data set contains 3 classes "
            ...
            features=tfds.features.FeaturesDict(
                {
                    "image": tfds.features.Tensor(shape=(4,), dtype=tf.float32),
...
institution = "University of California, Irvine, School of Information and Computer Sciences" }""",
    ...
    def _split_generators(self, dl_manager):
        ...
        iris_file = dl_manager.download(dl_url)
        print(iris_file)
        ...
        return [
            tfds.core.SplitGenerator(
                name="train",
                num_shards=1,
                gen_kwargs={"records": train_lines},
            ),
            tfds.core.SplitGenerator(
                name="test",
                num_shards=1,
                gen_kwargs={"records": test_lines},
            ),
        ]
    ...
    def _generate_examples(self, records):
        for record in records:
            elems = record.split(',')
            yield {
                "image": np.fromstring(' '.join(elems[:4]), dtype='f4', sep=' '),
                "label": elems[4]
            }
