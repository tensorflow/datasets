...
"""Imagenette: a subset of 10 easily classified classes from Imagenet (tench,
English springer, cassette player, chain saw, church, French horn, garbage
...
class Imagenette(tfds.core.GeneratorBasedBuilder):
  """Imagenette:A smaller subset of 10 easily classified classes from
  Imagenet."""
  ...
  def _split_generators(self, dl_manager):
    ...
    if size in _SIZES:
      variant = "imagenette%s.tgz"
      size = "" if size == "full-size" else "-" + size[:-2]
      variant = variant % size
      url = os.path.join(_URL_PREFIX, variant)
      path = dl_manager.download_and_extract(url)
      train_path = os.path.join(path, "imagenette", "train")
      val_path = os.path.join(path, "imagenette", "val")
    else:
