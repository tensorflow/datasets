...
It is inspired by the CIFAR-10 dataset but with some modifications.\
 In particular, each class has fewer labeled training examples than in \
CIFAR-10, but a very large set of unlabeled examples is provided to learn \
image models prior to supervised training. The primary challenge is to make use\
of the unlabeled data (which comes from a similar but different distribution\
 from the labeled data) to build a useful prior.
"""
