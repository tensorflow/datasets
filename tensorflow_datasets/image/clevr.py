...
class CLEVR(tfds.core.GeneratorBasedBuilder):
      ...
      splits.append(tfds.core.SplitGenerator(
          ...
          gen_kwargs={
              ...
              "question_file": os.path.join(
                  questions_path_dir, "CLEVR_{}_questions.json".format(split_name)),
  ...
  def _generate_examples(self, images_dir_path, question_file, scenes_description_file):
    image_paths = sorted([os.path.join(images_dir_path, filename)
                          for filename in tf.io.gfile.listdir(images_dir_path)])
    questions_json = json.load(tf.io.gfile.GFile(question_file))

    qlist = collections.defaultdict(list)
    for q in questions_json["questions"]:
      qlist[q["image_index"]].append({"question": q["question"], "answer": q["answer"]})
    ...
    for image_path, qna, scene in zip(image_paths, list(qlist.values()), scenes_json["scenes"]):
