...
class Cartoonset10k(tfds.core.GeneratorBasedBuilder):
  ...
  def _generate_examples(self, filepath):
    ...
    for file in tf.io.gfile.listdir(filepath):
      FeaturesDict = dict()
      _name, _type = file.split('.')
      if _type == 'png':
        image = tfds.core.lazy_imports.skimage.io.imread(
          filepath + '/' + _name + '.png')
        with tf.io.gfile.GFile(filepath + '/' + _name + '.csv', 'r') as f:
          reader = csv.reader(f, delimiter=',')
          for row in reader:
            FeaturesDict[row[0]] = row[1]
        FeaturesDict['image'] = image[:, :, :3] # Currently does not support alpha channels
        yield FeaturesDict
        ...
        for i in range(10):
            ...
            for file in tf.io.gfile.listdir(path):
                ...
                if _type == 'png':
                    image = tfds.core.lazy_imports.skimage.io.imread(
                        path + '/' + _name + '.png')
                    with tf.io.gfile.GFile(path + '/' + _name + '.csv', 'r') as f:
                        reader = csv.reader(f, delimiter=',')
                        for row in reader:
                            FeaturesDict[row[0]] = row[1]
                    FeaturesDict['image'] = image[:, :, :3]  # Currently does not support alpha channels
