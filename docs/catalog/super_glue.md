...
*   `boolq` (`v0.0.3`) (`Size: ?? GiB`): BoolQ (Boolean Questions, Clark et al.,
    2019a) is a QA task where each example consists of a short passage and a
    yes/no question about the passage. The questions are provided anonymously
...
*   `cb` (`v0.0.3`) (`Size: ?? GiB`): The CommitmentBank (De Marneffe et al.,
...
*   `copa` (`v0.0.3`) (`Size: ?? GiB`): The Choice Of Plausible Alternatives
...
*   `multirc` (`v0.0.3`) (`Size: ?? GiB`): The Multi-Sentence Reading
...
*   `record` (`v0.0.3`) (`Size: ?? GiB`): (Reading Comprehension with
...
*   `rte` (`v0.0.3`) (`Size: ?? GiB`): The Recognizing Textual Entailment (RTE)
...
*   `wic` (`v0.0.3`) (`Size: ?? GiB`): The Word-in-Context (WiC, Pilehvar and
...
*   `wsc` (`v0.0.3`) (`Size: ?? GiB`): The Winograd Schema Challenge (WSC,
...
*   `wsc.fixed` (`v0.0.3`) (`Size: ?? GiB`): The Winograd Schema Challenge (WSC,
    Levesque et al., 2012) is a reading comprehension task in which a system
    must read a sentence with a pronoun and select the referent of that pronoun
    from a list of choices. Given the difficulty of this task and the headroom
    still left, we have included WSC in SuperGLUE and recast the dataset into
